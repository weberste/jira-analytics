...
**Created**: 2026-01-29
**Status**: Draft
**Input**: User description: "CLI tool to estimate utilization per issue and per epic based on JIRA tickets matching a JQL query. The resulting data should be usesable to compare planned allocation (e.g., 30% maintenance, 20% new features) vs actual utilization without requiring developers to log time."
...
1. **Given** a JQL query and timeframe, **When** I run the utilization command, **Then** I receive a report listing each issue with its normalized time, developer, issue type, issue title, parent epic, and parent epic title.

2. **Given** a developer who worked on multiple issues in parallel on the same day, **When** I run the utilization command, **Then** their total time for that day is normalized to 7 hours, distributed proportionally across the issues.
...
- System MUST output per-issue data including: issue key, issue title, issue type, parent epic key (if any), parent epic title (if any), developer, raw hours, and normalized hours.
...
1. **Given** an issue that was "In Progress" for the entire day, **When** I calculate raw time for that day, **Then** it is credited with 7 hours of raw time before normalization.
...
5. **Given** a timeframe of Jan 1-15 and an issue in progress from Dec 28 to Jan 10, **When** I run the utilization command, **Then** only the time within Jan 1-10 is counted.
...
5. **Given** an issue that was unassigned during "In Progress" but later has a Developer or Assignee field set, **When** I rerun the analysis, **Then** the current Developer (if available)  or Assignee (if Developer not available) value is used and the issue is included in normalization (enabling retroactive data cleanup).
...
- System MUST support a `--show-incomplete` flag that lists the issue keys for both "issues with no time spent" and "issues without assignee" to allow further analysis and cleanup in JIRA.
...
**Independent Test**: Can be tested by running the utilization command and verifying output groups time by epic with percentages.

**Acceptance Scenarios**:

1. **Given** I run the utilization command (default), **When** the analysis completes, **Then** I see total normalized hours and percentage of total time per epic.

2. **Given** issues with no parent epic, **When** I run the utilization command, **Then** those issues are grouped under "No Epic" with their hours and percentage.
...
4. **Given** I run the utilization command with `--by-issue` flag, **When** the analysis completes, **Then** I see the detailed per-issue breakdown instead of epic aggregation.
...
1. **Given** I run the utilization command with `--output csv`, **When** the analysis completes, **Then** a CSV file is generated with columns: issue key, issue title, issue type, parent epic key, parent epic title, developer, date, normalized hours.
...
- **Normalized Time**: The adjusted time after scaling a developer's daily total to 7 hours.
...
- Parallel work by the same developer on the same day never exceeds 7 hours after normalization.
