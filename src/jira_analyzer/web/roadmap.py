"""Roadmap data fetching and processing for web interface."""
...
class RoadmapConfigError(AnalysisError):
...
def fetch_roadmap(jql: str, link_types: list[str] | None = None) -> RoadmapResult:
    ...
    try:
        ...
        raise InvalidConfigError(
            "Invalid configuration. Check your settings with "
            f"'jira-analyzer config show'. Error: {e}"
        )

    # Validate roadmap fields are configured
    if not config.roadmap_start_date_field or not config.roadmap_end_date_field:
        raise RoadmapConfigError(
            "Roadmap date fields are not configured. Add [roadmap] section to "
            "~/.jira-analyzer/config.toml with start_date_field and end_date_field."
        )

    start_field = config.roadmap_start_date_field
    end_field = config.roadmap_end_date_field
