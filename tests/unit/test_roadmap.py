...
from jira_analyzer.models import RoadmapEpic, RoadmapInitiative, RoadmapResult
...
def _make_config(start_field="cf_10015", end_field="cf_10016"):
    ...
    config = MagicMock()
    config.roadmap_start_date_field = start_field
    config.roadmap_end_date_field = end_field
...
class TestFetchRoadmap:
    ...
    @patch("jira_analyzer.web.roadmap.config_exists", return_value=False)
    def test_raises_when_no_config(self, mock_exists):
        with pytest.raises(ConfigNotFoundError):
            fetch_roadmap("type = Initiative")

    @patch("jira_analyzer.web.roadmap.load_config")
    @patch("jira_analyzer.web.roadmap.config_exists", return_value=True)
    ...
    def test_raises_when_roadmap_fields_missing(self, mock_exists, mock_load):
        config = MagicMock()
        config.roadmap_start_date_field = None
        config.roadmap_end_date_field = None
        mock_load.return_value = config
        with pytest.raises(RoadmapConfigError, match="not configured"):
            fetch_roadmap("type = Initiative")

    @patch("jira_analyzer.web.roadmap.JiraClient")
    @patch("jira_analyzer.web.roadmap.load_config")
    @patch("jira_analyzer.web.roadmap.config_exists", return_value=True)
...
class TestRoadmapRoutes:
    ...
    def test_get_roadmap_renders(self):
        with patch("jira_analyzer.web.routes.config_exists", return_value=True):
            resp = self.client.get("/roadmap")
        assert resp.status_code == 200
        assert b"Roadmap" in resp.data or b"roadmap" in resp.data

    def test_post_roadmap_requires_jql(self):
        with patch("jira_analyzer.web.routes.config_exists", return_value=True):
            resp = self.client.post("/roadmap", data={"jql": ""})
        assert resp.status_code == 400

    @patch("jira_analyzer.web.routes.fetch_roadmap")
    @patch("jira_analyzer.web.routes.roadmap_result_to_dict")
    @patch("jira_analyzer.web.routes.config_exists", return_value=True)
    def test_post_roadmap_success(self, mock_exists, mock_to_dict, mock_fetch):
        mock_result = MagicMock()
        mock_result.initiatives = []
        mock_fetch.return_value = mock_result
        mock_to_dict.return_value = {"initiatives": [], "timeline_start": "2026-01-01", "timeline_end": "2026-12-31", "jql_query": "test", "jira_url": "https://jira.example.com"}

        resp = self.client.post("/roadmap", data={"jql": "type = Initiative"})
        assert resp.status_code == 200
